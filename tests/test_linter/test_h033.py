...
test_data = [
    pytest.param(
        ...
        ([
            {"code": "H033", "line": "1:0", "match": '<form action="', "message": "Extra whitespace found in form action."},
            {"code": "H033", "line": "1:0", "match": '<form action=" {% ur', "message": "Extra whitespace found in form action."},
        ]),
        id="one",
    ),
    pytest.param(("<form action=\"{% url 'foo:bar' %}\" ...>...</form>"), ([]), id="one - no error"),
    pytest.param(
    ...
    pytest.param(("<form action=\"{% url 'foo:bar' %} {{ asdf}}\" ...>...</form>"), ([]), id="two - no error"),
    pytest.param(
        ("<form action=\" {% url 'foo:bar' %} \" ...>...</form>"),
        ([
            {"code": "H033", "line": "1:0", "match": '<form action="', "message": "Extra whitespace found in form action."},
    ...
    pytest.param(
        ...
        ([
            {"code": "H033", "line": "1:0", "match": '<form action="', "message": "Extra whitespace found in form action."},
            {"code": "H033", "line": "1:0", "match": '<form action=" {{ as', "message": "Extra whitespace found in form action."},
        ]),
        id="four",
    ),
    pytest.param(
        ("<form action=\"{% url 'foo:bar' %} \" ...>...</form>"),
        ([{"code": "H033", "line": "1:0", "match": '<form action="{% url', "message": "Extra whitespace found in form action."}]),
        id="five",
    ),
    pytest.param(
        ('<form action="asdf " ...>...</form>'),
        ([{"code": "H033", "line": "1:0", "match": '<form action="asdf "', "message": "Extra whitespace found in form action."}]),
        id="six",
    ),
    pytest.param(('<form action="asdf" ...>...</form>'), ([]), id="six - no error"),
    pytest.param(
        (
            '<form action="#"\n'
            '    method="get"\n'
