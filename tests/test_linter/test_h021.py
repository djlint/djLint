...
test_data = [
    pytest.param(
        ('<div style="asdf"></div>'),
        ([{"code": "H021", "line": "1:0", "match": "<div style=", "message": "Inline styles should be avoided."}]),
        id="simple",
    ),
    pytest.param(('<link href="https://fonts.googleapis.com/css?family=Roboto&display=swap" rel="stylesheet" />'), ([]), id="missing"),
    pytest.param(('<acronym title="Cascading Style Sheets">CSS</acronym>'), ([]), id="outside tag"),
    pytest.param(
        ('<div style="test {%"><div style="test {{">'),
        ([
            {"code": "H025", "line": "1:21", "match": '<div style="test {{"', "message": "Tag seems to be an orphan."},
            {"code": "H025", "line": "1:0", "match": '<div style="test {%"', "message": "Tag seems to be an orphan."},
        ]),
