"""Test for blank line after tag.

--blank-line-after-tag 'include,load'

poetry run pytest tests/test_config/test_blank_line_after_tag.py
"""
import pytest
...
test_data = [
    pytest.param(
        ...
        ({"blank_line_after_tag": "include,load, extends"}),
        id="one",
    ),
    pytest.param(
        (
    ...
    pytest.param(
        ("{% extends nothing %}\n" "\n" "<div></div>\n"),
        ("{% extends nothing %}\n" "\n" "<div></div>\n"),
        ({"blank_line_after_tag": "load, extends", "preserve_blank_lines": True}),
