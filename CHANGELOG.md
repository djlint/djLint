...
- New rule `H043` reports a `<button>` written without a `type`. A button with no type submits the form around it, so one meant to run a script reloads the page instead, and pressing Enter anywhere in the form triggers it. A type written inside a template block, as in `<button {% if a %}type="button"{% endif %}>`, counts.
- New rule `H044` reports a header row holding both `th` and `td` cells. The stray cell is read as data by a screen reader and styled unlike the columns beside it, and the mixture is legal html, so nothing else catches it. The empty `td` that opens a two-axis table's header row is allowed, and so is a row whose cells sit in sibling branches of a template block.
- New rule `H045` reports an `<iframe>` with no accessible name. A screen reader announces a frame by its name, and without one it reads out the url or says nothing, so there is no way to tell what is embedded before entering it. `title`, `aria-label` and `aria-labelledby` all count, as does a name written by a template tag.
- New rule `H046` reports a positive `tabindex`. One of them pulls its element to the front of the tab order, ahead of everything with none, so a single value rearranges the whole page for a keyboard user. `0` and `-1` are left alone, and so is a value written by a template tag.
- New rule `H047` reports `aria-hidden="true"` on an element that takes focus. The element leaves the accessibility tree but stays in the tab order, so a keyboard user lands on it and hears nothing. Hiding a decorative icon is the ordinary use of the attribute and is not reported, nor is a `disabled` control or one with `tabindex="-1"`.
- New rule `H048` reports an `aria-` attribute that ARIA does not define. A misspelled one does nothing at all, and nothing warns about it, so `aria-lable` can sit in a template while the control it was meant to name stays unnamed. A binding written by a framework, such as `:aria-label` or `[attr.aria-label]`, is left alone.
- New rule `H049` reports a viewport that stops the page being zoomed. `user-scalable=no`, and a `maximum-scale` below 2, prevent the enlargement that is the only way many people can read a page on a phone.
- New rule `H050` reports an element html no longer defines. `<center>`, `<font>`, `<big>`, `<strike>` and `<tt>` went when css took over presentation, and `<marquee>`, `<blink>` and `<nobr>` were never standard at all, so nothing says how a browser should lay any of them out and a stylesheet cannot target them the way it targets a class. Only the opening tag is reported, and a custom element whose name merely starts with one, such as `<font-picker>`, is left alone.
- New rule `H051` reports a `role` ARIA does not define for markup. An unknown role is dropped outright and the element keeps whatever meaning it already had, so `role="buton"` leaves a `<div>` a `<div>`, which is nothing to a screen reader, while the markup looks as though it had been given a purpose. The roles DPUB-ARIA and GRAPHICS-ARIA add count, and the abstract ones the specification says must not be written in markup, such as `landmark`, do not. A `role` holding a fallback list is checked word by word, and a value written by a template tag, or a framework binding such as `:role`, is left alone.
- New rule `H052` reports a `<meta http-equiv="refresh">` that reloads or redirects on a timer. The page moves out from under whoever is reading it, with no warning and no way to stop it, which fails WCAG 2.2.1, and a refresh of the same page throws away whatever was typed into it. A delay of zero is an immediate redirect rather than a timer and is not reported.
- New option `--quote-style` / `quote_style` chooses the quotes djLint writes around strings inside template tags, `double` (the default, and what it has always written) or `single`. `T002` asks for whichever is configured, so `{% include 'a.html' %}` is no longer reported in a project that writes single quotes. Html attributes are unaffected and stay with `H008`.
- New option `--sort-attributes` / `sort_attributes` orders a tag's attributes by name, with `id` first and `class` second. A tag whose attributes are guarded by a template tag, as in `<div {% if x %}a="1"{% endif %} b="2">`, keeps the order it was written in, since moving an attribute out of its branch would change the page.
...
- Formatting lowercases an attribute name written in another case, the same eleven names `H010` reports, so `<div CLASS="a">` is fixed rather than only complained about. A name the rule does not know, such as an svg `viewBox` or an angular input, keeps its case, and `--ignore-case` turns the whole thing off as it already does for tag names.
- Formatting writes an entity reference as the character it names, so `&copy;` becomes `©` and `&#8364;` becomes `€`, which is what `H023` asks for. The entities that have to survive are untouched: `&lt;`, `&amp;` and the rest carry syntax, an invisible one such as `&zwnj;` cannot be reviewed as a literal, and one that would complete a template delimiter, such as `&lbrace;` or `&percnt;`, would change what the template means. An entity naming nothing, such as the misspelled `&mdsah;`, is left for the rule to go on reporting, and a `<pre>`, `<textarea>`, `<script>` or `<style>` body, or the inside of a template tag, is left alone. `--no-entity-formatting` turns it off.
...
- Formatting lowercases a form's `method`, so `method="POST"` becomes `method="post"`. `method` is an enumerated attribute, so the page submits the same either way, and this is what `H029` asks for. A tag written inside a template tag's string, as in `{% set s = '<form method="POST">' %}`, is left alone.
...
- `T002` is on by default, and `--reformat` now writes the quotes it asks for. The formatter rewrites the arguments of `{% extends %}`, `{% include %}`, `{% with %}`, `{% trans %}` and `{% now %}` to the `--quote-style` in force, and a condition such as `{% if x == 'a' %}` with it, so one file does not spell the same string both ways. The rule leaves alone the one string it cannot rewrite, the one already holding the quote it would be rewritten to.
...
- A rule that looks for a tag no longer reads markup written inside an attribute value as a tag of its own. `<p title="a<br>b">` was reported by `H036` and `H017`, `<p title="an <img src=x>">` by `H013`, and `<p title="<a href='javascript:x()'>">` by `H022`. Affects `D004`, `J004`, `H006`, `H011`, `H013`, `H017`, `H018`, `H019`, `H020`, `H021`, `H022`, `H036` and `H043`.
...
- `H016` finds the document's title by where it sits rather than by how it is written. It accepted a `<title>` only when its attributes came from a hardcoded list and were double quoted, so `<title id='page-title'>` and an htmx `<title hx-swap-oob="true">` read as no title at all, while an svg `<title>`, which names a graphic, counted as the document's own. The report points at the `<html>` tag now rather than at the first twenty characters of the file.
...
- `H024` covers `<link rel="stylesheet" type="text/css">`, the commonest place the attribute is still written.
...
- `T001` and `T027` no longer read a delimiter written inside a string as the end of the tag, so `{{ x|default('}}') }}` is reported neither as needing padding nor as holding an unclosed string. A string that really does run past the tag is still reported.
...
- A template block opened and closed on one line no longer indents everything after it. `{% if a %}y{% endif %}<p>` left the `{% if %}` counted as open, so the first closing tag below it restored the indent to that phantom block's level and the rest of the file sat one level too deep until the next run pulled it back. Tags beyond `if`, `for`, `unless`, `block`, `with` and the two nunjucks async ones, such as `{% language 'de' %}text{% endlanguage %}`, were not recognised as balanced on a line at all.
...
- A `<pre>`, `<textarea>`, `<script>` or `<style>` opened and closed on one line no longer counts as closing a block opened earlier. `<pre>x</pre><script>` cancelled the block the `<script>` had just opened, so its body was indented as markup, and markup written after `</style>` on the closing line, as in `</style>z</b>`, went uncounted, so the element it closed kept its indent for the rest of the file. Both took a second run to settle.
...
- A rule's patterns are compiled once when the configuration is read rather than looked up again for every file, and the regex flags are worked out once instead of once per pattern per file. Matching a project's templates against the rule set went from 531 to 469 ms of processor time over 575 templates.
...
- Indenting is a quarter faster. Deciding whether a line closes a block the formatter must leave alone searched for every closing marker at once, and the two that can only match where the line starts cost the other markers their fast literal scan; searched apart, that check went from 199 to 16 ms of processor time over the lines of 575 templates.
- The search for what a line opens is skipped where the line holds neither a `<` nor a `{`, since it can then carry no markup and no template syntax, and the patterns the indenter builds from the configuration are built once for a run rather than once per file. Together those take another 7% off formatting.
- Putting short elements back on a single line is seven times faster, which takes close to a third off the formatter as a whole. The search let the whitespace after an opening tag be given back one character at a time, and every element that turned out too long to rejoin paid for the full retry, though the closing tag being looked for can only ever sit in one place. That pass went from 953 to 125 ms of processor time over 575 templates.
- The cache that answers what a line opens or closes holds 256 entries rather than 64, which lifted the hit rate of those lookups by about ten points for a tenth of a megabyte.
