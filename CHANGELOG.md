...
- An attribute value written without quotes is now quoted whatever else the tag holds, which is what `H011` asks for. It happened only when the tag was long enough to be spread over several lines, so `class=one` kept its shape or became `class="one"` depending on how much else was in the tag. A value carrying a quote of its own, as in `a=b'c`, is still left alone.
