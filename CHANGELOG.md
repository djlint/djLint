...
- Indenting is a further 18% faster. Deciding whether a line closes a block the formatter must leave alone searched for every closing marker at once, and the two that can only match where the line starts cost the other markers their fast literal scan; searched apart, that check went from 199 to 16 ms of processor time over the lines of 575 templates.
