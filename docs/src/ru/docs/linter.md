...
| H043 | Тег button должен иметь атрибут `type`.                                                                                     | -            |
...
Отключено по умолчанию; включается флагом --include=H043.

У `<button>` без `type` он равен `submit`, поэтому кнопка, написанная ради вызова скрипта, заодно отправляет окружающую форму и перезагружает страницу. Явно указанный тип избавляет от этого.
