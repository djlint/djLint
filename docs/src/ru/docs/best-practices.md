...
Если `format_attribute_template_tags` опция включена, условные атрибуты должны использовать теги без пробелов, например {% raw %}`{% if a -%}`{% endraw %} в nunjucks и jinja, чтобы удалить пробелы внутри тега.
...
Каждая отмеченная позиция — тег без пробелов.
