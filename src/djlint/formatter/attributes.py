...
def format_attributes(config: Config, html: str, match: re.Match[str]) -> str:
    ...
    for attr_grp in re.finditer(config.attribute_pattern, match.group(3).strip(), flags=re.VERBOSE):
        ...
        if attrib_name and attrib_name.lower() == "style":
            attrib_value = (";" + join_space).join([value.strip() for value in attrib_value.split(";") if value.strip()])

        elif attrib_name and attrib_name.lower() in {"srcset", "data-srcset", "sizes"}:
            # vw
            attrib_value = ("w," + join_space).join([value.strip() for value in attrib_value.split("w,") if value.strip()])
            # px
            attrib_value = ("x," + join_space).join([value.strip() for value in attrib_value.split("x,") if value.strip()])
