...
def print_output(config: Config, file_errors: Iterable[ProcessResult], file_count: int) -> int:
    ...
    tense_message = build_quantity(format_error_count) + " would be" if config.check else build_quantity_tense(format_error_count)
    reformat_success_message = f"{tense_message} updated."

    error_case = "error" if lint_error_count == 1 else "errors"
    lint_success_message += f"Linted {file_quantity}, found {lint_error_count} {error_case}."

    if print_blanks:
        echo()

    if not config.quiet and not config.stdin and (config.reformat or config.check):
        reformat_success_color = Fore.RED + Style.BRIGHT if (format_error_count) > 0 else Fore.BLUE
        echo(f"{reformat_success_color}{reformat_success_message}{Style.RESET_ALL}")

    if config.lint and not config.quiet:
        lint_success_color = Fore.RED + Style.BRIGHT if (lint_error_count) > 0 else Fore.BLUE
...
def build_output(error: Mapping[str, Iterable[LintError]], config: Config) -> int:
    ...
    errors = sorted(next(iter(error.values())), key=lambda x: tuple(int(i) for i in x["line"].split(":")))
    ...
    filename = build_relative_path(next(iter(error.keys())), config.project_root.resolve())

    if "{filename}" not in config.linter_output_format and not config.stdin:
        echo(f"{Fore.GREEN}{Style.BRIGHT}\n{filename}\n{Style.DIM}" + "".join("─" for _ in range(1, width)) + Style.RESET_ALL)

    for message_dict in errors:
        line = Fore.BLUE + message_dict["line"] + Style.RESET_ALL
        code = (Fore.RED if message_dict["code"][:1] == "E" else Fore.YELLOW) + message_dict["code"] + Style.RESET_ALL
        message = message_dict["message"]
        match = Fore.BLUE + re.sub(r"\s{2,}|\n", " ", message_dict["match"]) + Style.RESET_ALL

        echo(config.linter_output_format.format(filename=filename, line=line, code=code, message=message, match=match), err=False)
...
def build_check_output(errors: Mapping[str, Sequence[str]], config: Config) -> int:
    ...
    if not config.quiet and bool(next(iter(errors.values()))):
        echo(
            ...
            + Style.BRIGHT
            + "\n"
            + build_relative_path(next(iter(errors.keys())), config.project_root.resolve())
            + "\n"
            + Style.DIM
        ...
        for diff in next(iter(errors.values()))[2:]:
            echo(f"{color.get(diff[:1], Style.RESET_ALL)}{diff}{Style.RESET_ALL}", err=False)
...
def build_quantity_tense(size: int) -> str:
    """Count files in a list."""
    return str(size) + " file" + ("s" if size > 1 or size == 0 else "") + " " + ("were" if size > 1 or size == 0 else "was")


def build_stats_output(errors: Collection[Mapping[str, Iterable[LintError]] | None], config: Config) -> int:
    """Build output for linter statistics."""
    if not errors:
        return 0

    codes = tuple(code["code"] for error in errors if error for code in next(iter(error.values())))

    messages = {rule["rule"]["name"]: rule["rule"]["message"] for rule in config.linter_rules}

    echo()
    width, _ = shutil.get_terminal_size()
    echo(f"{Fore.GREEN}{Style.BRIGHT}Statistics{Style.RESET_ALL}\n{Style.DIM}{'─' * width}{Style.RESET_ALL}")
    ...
    if messages and codes:
        ...
        for code in sorted(Counter(codes).items()):
            ...
            echo(f"{Fore.YELLOW}{code[0]}{Fore.BLUE} {code_space}{code[1]}{Style.RESET_ALL} {count_space}{messages[code[0]]}")
